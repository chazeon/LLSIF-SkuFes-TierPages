    ...
    methods: {
        getLayout: function () {
            return {
                ...
                yaxis: { title: 'Live Score (pt)' },
                xaxis: { title: 'Event Score (pt)' },
                font: { family: 'Lato' },
        ...
        plotlyPlot: function (records) {
            let datasets = _.values(_.mapValues(records.records, tierRecords => {
                return {
                    x: _.map(tierRecords.data, record => record.event.score),
                    y: _.map(tierRecords.data, record => record.live.score),
                    type: 'line',
